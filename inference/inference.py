...
class Inference:
    ...
    def marketplace_url_list(self, x):
        url_list = []
        for i in range(len(x)):
            url_list.append(f"https://niftygateway.com/marketplace/collectible/{x[i]}")
        return url_list
    ...
    def proceed_inference(self, idx, itemid_json, itemid_imageurl_dict, transaction_data):
        ...
        for i in range(INFERENCE_NUM):
            recommended_item = itemid_dict[idx[i]]
            ...
            cand_nifty_obj_contract_address = list(set(cand_df.nifty_obj_contract_address))
            marketplace_url_list = self.marketplace_url_list(cand_nifty_obj_contract_address)
            likes = list(cand_df.nifty_obj_likes)[0]
            extension = list(cand_df.extension)[0]
            marketplace_url = marketplace_url_list[0]

            result[recommended_image_url] = {'extension': extension, 'likes':likes, 'url': marketplace_url}
